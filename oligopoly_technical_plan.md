...
- **Win detection + post-game completion:** **`winResolution.ts`** centralizes threshold checks via **`applyWinIfThresholdCrossed`**, invoked after tile purchases, auction settlements, and syndicate formation. Finalization persists **`winSummary`** and emits a **`game_won`** log payload explaining the win type, market value, threshold, and reason used by the in-game game-over banner. **`processGameCompletion`** (worker) updates human **`user_stats`**, **`user_ranks`**, achievements, and human-only KV leaderboard entries when **`game_over`** is persisted; AI seats are omitted from ranked rows and counted only in the aggregate leaderboard summary (`humanWins` / `aiWins`). Idempotency scans **`recent_games_json`** on **each** human participant (parsed JSON, matching **`gameId`**) before writing so completion work does not double-run if the canonical row order shifts.
