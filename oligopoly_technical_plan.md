...
- `game.ai_action` is emitted after each persisted AI-seat apply. Full envelope: `sentAt`, `gameId`, `aiPlayerId`, `personality`, `material`, `reason` (nullable ownership/capital/win-threshold cause enum, canonical `AiPresentationReasonSchema` in `@oligopoly/validation`), `softTurnEnd` (always present), `stateVersion`, optional `logCursor` / `summary` / `displayName`. The applied `GameAction` is **not** included — clients pace from presentation metadata plus the paired `game.action_applied` state, so sealed bid amounts and trade/contract terms never ride this fan-out.
