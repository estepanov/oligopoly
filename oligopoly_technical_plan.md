...
- **`roll_dice`** may omit **`result`** on the wire; the worker injects the authoritative dice **`result`** (via `rollDice()`) **and** **`pathChoiceDie`** before applying the action. (Clients always omit `result`; the AI runtime supplies its own deterministic dice.)
