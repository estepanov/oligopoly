...
- **Disruption cards:** landing on **DISRUPTION CARD** draws from **`disruptionDeckRemaining`** (shuffled at game start via **`buildDisruptionDeck`**). **`disruption_blitz`** optional rule draws/resolves 2 cards instead of 1 on disruption tiles. **FLASH CRASH** applies percentage losses plus a windfall to the lander. **BLACK MARKET RELAY** draws 2, keeps one deterministically, resolves it, and discards the other unseen; with **`disruption_blitz`**, it draws 4, keeps 2, resolves both, and discards the rest unseen. Resolution lives in **`disruptionEvents.ts`** (`blackMarketRelayParams`). Shared deterministic shuffle helpers live in **`deckShuffle.ts`**.
