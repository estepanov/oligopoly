...
- **Win detection + post-game completion:** **`winResolution.ts`** centralizes threshold checks via **`applyWinIfThresholdCrossed`**, invoked after tile purchases, auction settlements, and syndicate formation. Finalization persists **`winSummary`** and emits a **`game_won`** log payload explaining the win type, market value, threshold, and reason used by the in-game game-over banner. **`processGameCompletion`** (worker) updates human **`user_stats`**, **`user_ranks`**, achievements, and human-only KV leaderboard entries when **`game_over`** is persisted; AI seats are omitted from ranked rows and counted only in the aggregate leaderboard summary (`humanWins` / `aiWins`). **`leaderboard:summary`** is merged via **`LeaderboardSummarySchema.safeParse`** with zero defaults in both **`incrementLeaderboardSummary`** and **`GET /api/leaderboard/*`** reads so corrupt or legacy KV blobs never throw after other work succeeds. The same completion path uses non-throwing JSON parsing with Zod-backed **`safeParseJson`** for **`leaderboard:wins`** / **`leaderboard:completions`** row lists (**`LeaderboardWinsEntrySchema`** / **`LeaderboardCompletionsEntrySchema`**), **`games.player_ids_json`** (**`z.array(z.string())`**), and **`user_stats.recent_games_json`** (**`RecentGameSummarySchema`** arrays) so malformed blobs reset to empty lists instead of unsafe casts; when **`player_ids_json`** parses to an empty list, **`processGameCompletion`** falls back to **`state.players`** seat ids so aggregate **`leaderboard:summary`** updates still run. Idempotency scans **`recent_games_json`** on **each** human participant (parsed JSON, matching **`gameId`**) before writing so completion work does not double-run if the canonical row order shifts.
