...
- **`applyAction`** in `@oligopoly/shared` (`gameStateMachine`) is the primary transition for **`POST /api/games/:id/action`**. Typed game errors use **`GameErrorKeys`** in `@oligopoly/validation`. Successful actions append authoritative **`player_state_changed`** log entries for visible player deltas such as Capital, owned/mortgaged tiles, development tokens, Action Points, trustworthiness, debt, position, regulation status, and Syndicate membership. The allowed delta keys are defined authoritatively as the **`PLAYER_STATE_CHANGE_FIELD_KEYS`** tuple in `@oligopoly/validation` (alongside strict **`PlayerStateChangesBodySchema`**, which rejects unknown keys in `changes` at parse time); the engine and web each mirror that contract with keyed registries (**`PLAYER_STATE_CHANGE_REGISTRY`**, **`PLAYER_STATE_LOG_PARTS`**) so iteration and formatting stay aligned without maintaining three independent string lists. The engine builds snapshots and diffs only through **`PLAYER_STATE_CHANGE_REGISTRY`** (per-field `snapshot` + `diff`), and the web log formatter uses **`PLAYER_STATE_LOG_PARTS`**, so new fields stay a coordinated validation + engine + UI change.
