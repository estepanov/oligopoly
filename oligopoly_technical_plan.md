...
- **Syndicates (foundation):** **`form_syndicate`** creates a permanent syndicate (2+ members, 1 AP unless Founding Partner). **`syndicate.ts`** provides shared ownership helpers for sector control, hub/utility counts, and rent exemption between members. **`checkWinConditions`** detects syndicate wins at 60% market value via **`findSyndicateWinnerId`**. Charter, votes, revenue split, and coordination phase remain future work.
