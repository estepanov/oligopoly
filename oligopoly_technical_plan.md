...
- **Industry Affinity (partial wiring):** **`affinityAssignments`** is server-only at game start. **`affinity.ts`** exposes lookup helpers. **Lean Manufacturing** discounts development costs in **`develop_tile`**. **PropTech Pioneer** discounts mortgage redemption in **`redeem_tile`**. **Last Mile Logistics** awards **`DIAGONAL_TRAVERSE_BONUS`** (30 Capital) when a player exits the Diagonal Express at the far end during movement. Active affinities (**Biotech IP**, **Consumer Insights**) remain future work.
