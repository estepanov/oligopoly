...
- `game.ai_action` is emitted after each persisted AI-seat apply. Full envelope: `sentAt`, `gameId`, `aiPlayerId`, `personality`, `action` (the applied `GameAction`), `material`, `reason` (nullable ownership/capital/win-threshold cause enum), `softTurnEnd`, `stateVersion`, optional `logCursor` / `summary` / `displayName`.
