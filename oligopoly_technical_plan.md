...
- **Negotiation (in-game):** **`start_negotiation`** (1 AP), **`sign_contract`**, **`break_handshake`** wired in **`negotiationActions.ts`** with in-state threads and trust penalties on expiry. Binding contract enforcement uses **`negotiation.ts`** `isActionBlockedByContracts`.
