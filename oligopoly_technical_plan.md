...
- **`roll_dice`** is **server-authoritative**: the worker injects the dice **`result`** from crypto RNG (`rollFairDice()`) **and** **`pathChoiceDie`** before applying the action. Real (deployed) players cannot choose favorable rolls; a client-supplied `result` is honored **only on local/test origins** (`isLocalDevRequest`) so deterministic integration tests can drive specific dice. The AI runtime supplies its own deterministic dice via the engine directly.
