...
- **`@oligopoly/worker` JSON helpers:** **`packages/worker/src/lib/jsonParse.ts`** exposes **`safeParseJson`**, **`safeJsonParse`**, and **`safeParseJsonArray`** for D1/KV string fields (admin routes, **`processGameCompletion`**, **`GET /api/leaderboard/*`**, and similar) so non-throwing parse fallbacks stay centralized instead of per-file copies. **`packages/worker/src/lib/recentGamesJson.ts`** holds lenient **`recentGamesJsonContainsGameId`** (idempotency) and **`sanitizeRecentGamesFromStorage`** (strict per-row filtering before rewrite) for **`user_stats.recent_games_json`**.
...
- `game.ai_action` is emitted after each persisted AI-seat apply. Full envelope: `sentAt`, `gameId`, `aiPlayerId`, `personality`, `action` (the applied action, **redacted** server-side — a sealed `auction_bid` omits `amount`, `propose_trade`/`counter_trade` omit `gives`/`receives`, `propose_contract` omits `terms`; validated against a permissive `{ type: string, ...passthrough }` shape, not the full `GameActionSchema` union), `material`, `reason` (nullable ownership/capital/win-threshold cause enum, canonical `AiPresentationReasonSchema` in `@oligopoly/validation`), `softTurnEnd` (always present), `stateVersion`, optional `logCursor` / `summary` / `displayName`.
...
- `stateVersion` is a general persist counter on `GameState`, not an AI-only field: it increments on every successful game state persist, human or AI action alike, and is what clients use to order/pair `game.action_applied`/`game.ai_action` broadcasts.
...
- **`applyAction`** in `@oligopoly/shared` (`gameStateMachine`) is the primary transition for **`POST /api/games/:id/action`**. Typed game errors use **`GameErrorKeys`** in `@oligopoly/validation`. Successful actions append authoritative **`player_state_changed`** log entries for visible player deltas such as Capital, owned/mortgaged tiles, development tokens, Action Points, trustworthiness, debt, position, regulation status, and Syndicate membership. The allowed delta keys and per-field Zod sub-schemas live in **`PLAYER_STATE_CHANGE_FIELD_SCHEMAS`** in `@oligopoly/validation`; **`PLAYER_STATE_CHANGE_FIELD_KEYS`** and strict **`PlayerStateChangesBodySchema`** are derived from that table (no parallel field list). The engine and web mirror the contract with keyed registries (**`PLAYER_STATE_CHANGE_REGISTRY`**, **`PLAYER_STATE_LOG_PARTS`**); **`playerStateChangeContract.test.ts`** asserts schema ↔ keys alignment, engine **`buildPlayerStateChangesBody`** output validity, and web formatter coverage per field. The engine builds snapshots and diffs only through **`PLAYER_STATE_CHANGE_REGISTRY`** (per-field `snapshot` + `diff`), and the web log formatter uses **`PLAYER_STATE_LOG_PARTS`**, so new fields stay a coordinated validation + engine + UI change.
...
- The web client merges realtime snapshots with the last authenticated HTTP snapshot via **`mergeAuctionClientView`** so sealed-bid **`mySubmission`** survives broadcasts that omit it; the merge applies only when **`previous.gameId`** matches **`incoming.gameId`** (or **`gameId`** is missing on a side), so navigating between games cannot splice another game's viewer-only auction fields.
...
- **Win detection + post-game completion:** **`winResolution.ts`** centralizes threshold checks via **`applyWinIfThresholdCrossed`**, invoked after tile purchases, auction settlements, and syndicate formation. Finalization persists **`winSummary`** and emits a **`game_won`** log payload explaining the win type, market value, threshold, and reason used by the in-game game-over banner. **`processGameCompletion`** (worker) updates human **`user_stats`**, **`user_ranks`**, achievements, and human-only KV leaderboard entries when **`game_over`** is persisted; AI seats are omitted from ranked rows and counted only in the aggregate leaderboard summary (`humanWins` / `aiWins`). **`leaderboard:summary`** is read via **`parseLeaderboardSummaryFromKv`** (`**safeParseJson**` + **`LeaderboardSummarySchema`**); ranked **`leaderboard:wins`** / **`leaderboard:completions`** row lists use **`safeParseJsonArrayElements`** with **`LeaderboardWinsEntrySchema`** / **`LeaderboardCompletionsEntrySchema`** so only invalid elements are dropped (including **`incrementLeaderboardSummary`**, **`GET /api/leaderboard/*`**, and the batched completion flush) so corrupt KV never throws. Per-human **`leaderboard:wins`** / **`leaderboard:completions`** updates are accumulated during the D1 loop and applied in **one read-modify-write per KV key after `db.batch`**, instead of per-player KV round-trips inside the loop. D1 idempotency still uses **`recentGamesJsonContainsGameId`**, but leaderboard KV uses separate per-game step markers (`leaderboard:completion:{gameId}:wins|completions|summary`) so a later run can repair KV after a partial flush without double-applying D1 stats or re-incrementing already-marked KV steps. **`games.player_ids_json`** uses **`safeParseJson`** with **`z.array(z.string())`**; when it parses to an empty list, completion falls back to **`state.players`** seat ids so aggregate summary updates still run. **`user_stats.recent_games_json`**: idempotency uses **`recentGamesJsonContainsGameId`** (lenient JSON array scan for a matching **`gameId`**) so a single corrupt history row cannot hide an already-recorded game and cause a double apply; persisted history is rebuilt with **`sanitizeRecentGamesFromStorage`**, which keeps only elements that pass **`RecentGameSummarySchema`**. Idempotency scans **each** human participant before writing so completion work does not double-run if the canonical row order shifts.
...
- **Negotiation (in-game):** **`start_negotiation`** (1 AP), **`propose_contract`**, **`sign_contract`**, **`break_handshake`** wired in **`negotiationActions.ts`** with in-state threads and trust penalties on expiry. **`propose_contract`** registers **`activeContracts`** with proposer signature; counterparty signs via **`sign_contract`**. Binding contract enforcement uses **`negotiation.ts`** `isActionBlockedByContracts`. Full trade offers use **`tradeActions.ts`**: **`propose_trade`** costs 1 AP and is turn-scoped to the action phase. **`accept_trade`** and **`reject_trade`** are global recipient responses valid in any phase (even off-turn); **`counter_trade`** is also recipient-driven and globally routed (the recipient need not be the active player) but is additionally gated to the action phase per the game rules. None of the three responses cost an action point. Accepted trades atomically transfer Capital plus unmortgaged properties through **`transferTileOwnership`**, re-check live ownership/capital/contract blockers before mutation, and invoke **`applyWinIfThresholdCrossed`**. Pending offers default to 5-minute expiry, counter chains are capped at two, and **`expirePendingTradeOffers`** marks timed-out offers expired with no asset transfer. **`chooseAiAction`** handles AI trade participation by responding to pending offers addressed to AI players and proposing one simple cash-for-property trade during an AI action phase when AP/capital permit; **`GameRoom`** and **`POST /api/games/:id/action`** run the AI loop so non-turn AI trade responses happen promptly. Trade mutation persistence passes the loaded **`state_json`** into **`persistGameActionResult`** and uses an optimistic **`UPDATE ... WHERE state_json = ?`** guard so stale global actions fail with **`game.state_conflict`** instead of overwriting a newer game state. Trade log entries are public game-log events for proposals, counters, acceptances, rejections, and expirations; **`GameActionLog`** formats Capital and property terms. **`GameRoom`** schedules the earliest pending trade expiry as **`timerKind: "trade_offer"`** via **`syncGameRoomTimer`** while preserving any underlying turn deadline; **`applyTradeOfferExpiry`** persists the timeout rejection.
- **Web client (game detail):** **`GamePlayControls`** exposes **`RateCardPanel`** (syndicate admin **`set_rate_card`** on own turn when qualified) alongside advanced action-phase controls (**`form_syndicate`**, **`start_negotiation`**, **`propose_contract`**, **`sign_contract`**, **`propose_trade`**, **`accept_trade`**, **`reject_trade`**, **`counter_trade`**, **`pay_debt`**, **`initiate_auction`**, **`use_affinity`**, disruption nullify) via **`ActionPhaseExtras`**. **`TradeNegotiationPanel`** renders a private trade desk for involved players, keeps incoming accept/reject controls visible outside the recipient's turn, and gates proposal/counter composition to action turns. **`BoardGrid`** shows develop/mortgage/redeem economics only for tiles the **viewer** owns (public tile details still come from config + **`tileState`**). **`PlayerSummaryPanel`** calls **`getTileEconomics`** with the **listed player** as the economics subject so each row reflects that owner's capital and affinities, while **`myPlayerId`** remains the viewer for redacted affinity context.
