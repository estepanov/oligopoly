...
- **`applyAction`** in `@oligopoly/shared` (`gameStateMachine`) is the primary transition for **`POST /api/games/:id/action`**. Typed game errors use **`GameErrorKeys`** in `@oligopoly/validation`. Successful actions append authoritative **`player_state_changed`** log entries for visible player deltas such as Capital, owned/mortgaged tiles, development tokens, Action Points, trustworthiness, debt, position, regulation status, and Syndicate membership. The allowed delta keys are enumerated once as **`PLAYER_STATE_CHANGE_FIELD_KEYS`** in `@oligopoly/validation` (alongside strict **`PlayerStateChangesBodySchema`**, which rejects unknown keys in `changes` at parse time). The engine builds snapshots and diffs only through its keyed **`PLAYER_STATE_CHANGE_REGISTRY`** (per-field `snapshot` + `diff`), and the web log formatter mirrors that with **`PLAYER_STATE_LOG_PARTS`**, so new fields stay a coordinated validation + engine + UI change.
