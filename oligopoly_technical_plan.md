...
- **Optional market event cards:** all ten registry IDs resolve in **`marketEvents.ts`** `resolveSpecificEffect` (leveraged buyout, corporate espionage, short squeeze, supply chain crisis, sovereign wealth fund, venture capital boom, algorithmic flash trade, regulatory amnesty, dark pool transfer, synthetic CDO, black swan event).
